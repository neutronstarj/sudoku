...
class FirstAvailable(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        #retrive all cells from grid
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                #check if the domain has more than one value
                if len(cells[i][j])>1:
                       return(i,j)
        return None # if no variable with multiple values is found
...
class MRV(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        #initialize a avariable to track smallest domain size
        cells= grid.get_cells()
        width=grid.get_width()
        for i in range (width):
            for j in range(width):
                #check if the cell is unassigned and have smallest domain
                if len(cells[i][j])<min_domain and len(cells[i][j])>1:
                    min_domain=len(cells[i][j])
                    selected_var=(i,j) #update
        return selected_var
...
class AC3:
    ...
    def pre_process_consistency(self, grid):
        ...
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                #check if the domain has single value assigned 
                if len(grid.get_cells()[i][j]) == 1:
                    Q.add((i, j))

        # Call consistency method to enforce arc consistency
    ...
    def consistency(self, grid, Q):
        ...
        while Q:
            #retrive and process the next variable from queue
            var =Q.pop()
            row,col=var
            # Remove assigned values from the domains in the same row, column, and unit
            v_row,failure_row = self.remove_domain_row(grid,row,col)
            if  v_row is not None:
                # Update the queue with newly assigned variables from the row
                Q.update(v_row)
            v_col,failure_col= self.remove_domain_column(grid,row,col)
            if v_col is not None:
                Q.update(v_col)
            v_unit,failure_unit = self.remove_domain_unit(grid,row,col)
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        """
        Implements backtracking search with inference using AC3.
        """
        ac3=AC3()
        if grid.is_solved():
            return grid  
        ...
        i, j = var # Get the row and column of the selected variable
        # Iterate over possible values in the variable's domain
        for d in grid.get_cells()[i][j]:
            
            if grid.is_value_consistent(d, i, j):  # Check d is consistent 
                copy_grid = grid.copy()
                copy_grid.get_cells()[i][j] = d  

                #call recursively to solve
...
def main():
    ...
    for p in problems:
        ...
        # pre consistency should only call once 
        ac3 = AC3()
...
#MRV 
        var_selector = MRV()
        sol = backtracking_solver.search(g, var_selector)
        end = time.time()
        ...
        if sol is not None:
            ...
            sol.print()
            print('Is the current grid a solution? ', sol.is_solved())
        else:
            print("No solution exists for the given Sudoku puzzle.")
            

        # Calculate running time for FirstAvailable heuristic for the same problem
        g = Grid()
        g.read_file(p)
        ac3 = AC3()
        ac3.pre_process_consistency(g)
    ...
    plotter = PlotResults()
    plotter.plot_results(mrv_running_times, fa_running_times, "Running time (MRV)", "Running time (FA)", "running time")
